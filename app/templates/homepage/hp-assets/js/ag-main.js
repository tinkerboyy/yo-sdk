...
/*-----------------------
	...
	$('.enter').on('mouseover', btHover);
	$('.enter').on('mouseleave', btOut);
	$('.close a').on('mouseover', btHover);
	$('.close a').on('mouseleave', btOut);
	
	function  btHover(e){
		e.preventDefault();
		TweenLite.to($(this),0.2, {css:{"background-color":"#38923E"}});
...
/*-----------------------
	...
	function shareEv(e){
		...
		if($(this).attr('href')=='#facebook'){
			window.open("#","Facebook Share","height=300, width=600,scrollbars=yes");
			_gaq.push(['_trackPageview', '/Facebook-Share']);
		}else if($(this).attr('href')=='#twitter'){
			window.open("#","Twitter Share","height=300, width=600,scrollbars=yes");
			_gaq.push(['_trackPageview', '/Twitter-Share']);
		}else if($(this).attr('href')=='#linkedin'){
			window.open("#","Linkedin Share","height=400, width=600,scrollbars=yes");
			_gaq.push(['_trackPageview', '/Linkedin-share']);
		}else if($(this).attr('href')=='#google'){
			window.open("#","Google+ Share","height=300, width=600,scrollbars=yes");
...
/*-----------------------
	...
	function showVideo(){
		...
		$('.video #youtube').append('<iframe id="player" src="//www.youtube.com/embed/QtRIU_o01xk?html5=1?rel=0&autoplay=1" allowfullscreen></iframe>');
...
/*-----------------------
	...
	if( Modernizr.touch){
		...
		//$('#login-tab .apear:eq(0)').waypoint(function() {show($('#login-tab .apear:eq(0)'),0)}, { offset: '90%' });
		
		//$('#whatisactasone .apear:eq(0)').waypoint(function() {show($('#whatisactasone .apear:eq(0)'),0)}, { offset: '90%' });	
//		$('#whatisactasone .apear:eq(1)').waypoint(function() {show($('#whatisactasone .apear:eq(1)'),0)}, { offset: '80%' });	
//		$('#whatisactasone .apear:eq(2)').waypoint(function() {show($('#whatisactasone .apear:eq(2)'),0)}, { offset: '80%' });

		$('#whatisactasone .apear:eq(0)').waypoint(function() {show($('#whatisactasone .apear:eq(0)'),0)}, { offset: '90%' });
		$('#whatisactasone .apear:eq(1)').waypoint(function() {show($('#whatisactasone .apear:eq(1)'),0)}, { offset: '80%' });	
		$('#whatisactasone .apear:eq(2)').waypoint(function() {show($('#whatisactasone .apear:eq(2)'),0.25)}, { offset: '80%' });	
		$('#whatisactasone .apear:eq(3)').waypoint(function() {show($('#whatisactasone .apear:eq(3)'),0.5)}, { offset: '80%' });	
		
				$('#relevance .apear:eq(0)').waypoint(function() {show($('#relevance .apear:eq(0)'),0)}, { offset: '90%' });	
		$('#relevance .apear:eq(1)').waypoint(function() {show($('#relevance .apear:eq(1)'),0)}, { offset: '80%' });	
		$('#relevance .apear:eq(2)').waypoint(function() {show($('#relevance .apear:eq(2)'),0)}, { offset: '80%' });			
...
$(window).scroll(function(e){
			...
			if($(window).scrollTop()<=$('#whatisactasone').offset().top-perc){
				navSelector('video');
					
			
			}else if ($(window).scrollTop()<=$('#whynow').offset().top-perc){
				navSelector('whatisactasone');
